...
from general import game_constants, SpecialChars, RenderUpdatesDraw, Box, Platform, Score, Word, GetFont
...
class Health:
    ...
    def draw(self,screen):
        if self.redraw:
            lastx = 5
            dirty = []
            for i in xrange(self.current_hearts): # Full hearts
                dirty += [screen.blit(self.full,self.full.get_rect(left = lastx, top = 5))]
                lastx = lastx + self.full.get_width() + 2
            for i in xrange(self.current_hearts,self.max_hearts): # Empty hearts
...
class InstructionsScreen:
    def __init__(self,screen):
        ...
        import os
        self.image = pygame.image.load(os.path.join('.','data','gunstar','gunside1.png')).convert_alpha()
        self.image = pygame.transform.scale2x(pygame.transform.scale2x(self.image)) # x4!
        
        self.soldier = pygame.image.load(os.path.join('.','data','soldier','rest1.gif')).convert_alpha()
        self.chopper = pygame.image.load(os.path.join('.','data','copter','fly1.gif')).convert_alpha()
        self.ghost = pygame.image.load(os.path.join('.','data','ghost','right1.gif')).convert_alpha()
        self.showing = True
    ...
    def draw(self):
        ...
        enemy_list_text.append("GHOSTS can phase through platforms.")
        enemy_list_text.reverse()
        enemy_list = [GetFont(16).render(text, 1, (250,250,250)) for text in enemy_list_text] 
        
        instructions_text = ["Type the words that appear over enemies to defeat them."]
        instructions_text.append("Type the character that appears on a platform to jump to it.")
        instructions_text.append("Press CTRL to deselect a word.")
        instructions_text.append("Press space to turn around.")
        instructions_text.append("Press return to continue.")
        instructions_text.reverse()
        instructions = [GetFont(16).render(text, 1, (250,250,250)) for text in instructions_text] 
...
class MainGameScene:
                ...
                if platform not in self.screenstatics: # This platform wasn't on the screen: is it now?
                    ...
                    if not platform.rect.colliderect(self.camera):
                        ...
                        if hasattr(platform,'word'):
                            ...
                            platform.word = None
        for object in self.powerups.sprites():
            if not object.rect.colliderect(self.camera.inflate(game_constants.w,0)):
                object.kill()
    ...
    def tick(self,elapsed):
        ...
        for object in self.actives:
            object.tick(self.player.rect.center)
        for object in self.powerups:
            object.tick()
        for object in self.screenstatics: # Objects on the screen
            # Only platforms have a 'word' attribute
            if hasattr(object,'word'):
                # Will it be possible to get to this object without having to walk on air
                object.reachable = is_reachable(object,self.player.rect,self.screenstatics)
        self.player.tick()
