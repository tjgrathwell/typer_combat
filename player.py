...
from general import Anim, game_constants, RenderUpdatesDraw
...
class Player(pygame.sprite.Sprite):
    ...
    def setanim(self): # Set animation from state and direction
        # Basically just a complicated if-else map between current state and animations
        # This could be replaced by having a self.animleft, self.animright and indexing them by state.
        if self.state != States.shooting:
            if self.direction == 'l':
                if self.state == States.jumping:
                    ...
                    self.current_anim = self.hitleft
                ...
                if self.state == States.jumping:
                    ...
                    self.current_anim = self.hitright
    ...
    def shoot(self, selected):
        ...
        for i in xrange(shotcount):
            ...
            self.bullets.add(Bullet(self,self.weapon,(player[0]-nx*4,player[1]-ny*4),(nx,ny),selected,ttl))
